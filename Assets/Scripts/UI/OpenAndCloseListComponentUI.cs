...
public class OpenAndCloseListComponentUI : MonoBehaviour
{
    [SerializeField] private GameObject  childPanel;
    [SerializeField] private OpenListComponentUIManager openListComponentManager;
    ...
    public void OpenUI()
    {
        if (!isCurrentlyOpen)
        {
            ...
            Debug.Log($"{childPanel.name} UI is now opened!");
        }
    }
    ...
    public void CloseUI()
    {
        if (isCurrentlyOpen)
        {
            ...
            openListComponentManager.SetIsOpenState(false);
            this.childPanel.SetActive(false);
            Debug.Log($"{childPanel.name} UI is now closed!");
        }
    }
